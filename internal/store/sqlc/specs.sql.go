...
func (q *Queries) ListDistinctFeatureNamesForProduct(ctx context.Context, productID string) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, listDistinctFeatureNamesForProduct, productID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
