defmodule AcaiWeb.UserAuthTest do
  ...
  describe "require_sudo_mode/2" do
    # dashboard.AUTH.2
    test "dashboard.AUTH.2 allows users that authenticated within the last 20 minutes", %{
      conn: conn,
      user: user
    } do
      nineteen_minutes_ago = DateTime.utc_now(:second) |> DateTime.add(-19, :minute)
      user = %{user | authenticated_at: nineteen_minutes_ago}
    ...
    # dashboard.AUTH.2
    # dashboard.AUTH.2-1
    test "dashboard.AUTH.2-1 redirects when authentication is older than 20 minutes", %{
      conn: conn,
      user: user
    } do
